        ...
        try:
            ...
            with ui.card().classes(
                ...
                with ui.card_section().classes("w-full"):
                    ui.label(f"Witaj, {user.email}!").classes("text-4xl w-full text-center")

                with ui.card_section().classes("w-full h-full"):
                    PaymentForm(user_info)

                # Wyszukiwarka kont
                search_query = ui.input(label="Wyszukaj konto po walucie lub ID").classes("my-4 w-full")
                    ...
                    with result_container:
                        with ui.scroll_area().classes("max-h-96 w-full p-2"):
                            if not filtered_accounts:
                                ui.label("Nie znaleziono konta ¯\\_(ツ)_/¯").classes("text-center text-red-500")
                                return

                            for account in filtered_accounts:
                                with ui.card().classes("bg-blue-100 text-black w-full my-2 p-2 rounded-lg shadow-sm"):
                                    ui.label(
                                        f"Historia transakcji dla konta {account.currency} ({account.id})").classes(
                                        "font-bold text-md")

                                    transactions = get_accounts_transaction_history(account.id)
                                    if not transactions:
                                        ui.label("Brak transakcji 💤").classes("text-sm italic text-gray-500")
                                    else:
                                        with ui.scroll_area().classes("max-h-40 w-full mt-1"):
                                            for t in transactions:
                                                direction = "⬆️ Wysłano" if t.source_account_id == account.id else "⬇️ Otrzymano"
                                                amount = f"{t.amount_numeric:.2f} {account.currency}"
                                                counterparty_id = t.target_account_id if direction == "⬆️ Wysłano" else t.source_account_id
